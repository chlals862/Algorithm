...
public class 인구이동 {
	...
	static int L,R;
	static int count,sum,testCount,result;//연합을 맺은 국가의 수, 연합의 인구 수
	static int[][] map;
	static int[][] border;
	static boolean flag;
	...
	while(true) {
		for(int row=0;row<N;row++) {
			for(int col=0;col<N;col++) {
				...
				if(border[row][col] == 0 && visit[row][col] == false) {//연합을 가지지 않은 국가 조사
				...
				//연합을 가진 국가들 
				if(border[row][col] == num-1 && visit[row][col] == true) {
					//result = Math.round(sum/count);
				//	map[row][col] = result;
					time++;
					if(border[row][col] == 1) {
						System.out.println("2번 연합 국가수 = " + count);
						result = Math.round(sum/count);
						map[row][col] = result;
						System.out.println("result = " + result);
					}
					//System.out.println("border[row][col] = " + border[row][col] + " num-1 = " + (num-1)); 
				}
		...
		view2();
		//view2();
		//System.out.println("연합 수 = " + list.size());
		//System.out.println("result = " + result);
